module.exports = (grunt) ->
  ...
  grunt.initConfig
    ...
    uglify:
      ...
      integration:
        options: {}
        files:
          "build/<%= currentBuild %>/simple-scan.min.js": ["build/<%= currentBuild %>/simple-scan.js"]
    ...
    exec:
      ...
      integration:
        command: "vulcanize --csp -i smoke.html -o build/<%= currentBuild %>/simple-scan.html"
        stdout: true
        stderr: true
    ...
    replace:
      integration:
        src: ["build/<%= currentBuild %>/simple-scan.html"]
        dest: "build/<%= currentBuild %>/simple-scan.html"
        replacements: [
    ...
    copy:
      ...
      desktopFinal:
        files: [
          {expand: true, src: ['_tmp/desktop/releases/simple-scan/linux64/simple-scan/**'], dest: 'build/<%= currentBuild %>/'},
        ]
    ...
    htmlmin:
      integration:
        ...
        files: # Dictionary of files
          "build/integration/simple-scan.html": "build/integration/simple-scan.html"

    clean:
      integration: ["build/<%= currentBuild %>"]
      postIntegration: ["build/<%= currentBuild %>/platform.js", "build/<%= currentBuild %>/simple-scan.js"]
  ...
  #Builds
  @registerTask 'build', 'Build simple-scan for the chosen target/platform etc', (target = 'browser', subTarget='standalone') =>
